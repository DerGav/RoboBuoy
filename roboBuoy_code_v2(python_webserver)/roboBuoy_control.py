...
class RoboBuoy(object):
	...
	def setup(self):
		...
		self.voltageMonitor = VoltageMonitor.VoltageMonitor(22)
		LeakDetection.setup()
	...
	def start(self):
		while self.run:
			...
			# ############################################################################
			#
			self.leak = LeakDetection.checkForLeak()
			...
			# ############################################################################
			#
			self.battery_voltage = self.voltageMonitor.read_voltage()
			# #TODO:handle voltage
			# 	#warning to page
			# 	#shutdown motors?
			# 	#shutdown pi?
			...
			self.mag_heading = Magnetometer.getTrueHeadingTiltComp(self.lsm) #current heading - in degrees
			self.current_lat = self.gpsc.fix.latitude
			self.current_long = self.gpsc.fix.longitude
			self.satellites_used = str(self.gpsc.satellitesUsed)
			#
			# ############################################################################
			# # calculate distance
			# ############################################################################
